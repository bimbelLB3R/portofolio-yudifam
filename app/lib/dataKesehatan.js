...
export async function getFilteredKesehatanData(query, currentPage) {
  ...
  try {
      ...
      const filteredRows = rows
        .filter(
          (item) =>
            ...
            item.jenis_penyakit.toLowerCase().includes(lowerQuery)
        )
        .sort((a, b) => {
          // Pastikan bahwa Anda memiliki properti yang sesuai untuk tanggal di dalam objek 'item'
          const dateA = a.id_penyakit;
          const dateB = b.id_penyakit;
          return dateB - dateA;
    ...
    const penyakits = filteredRows;

    return penyakits;
  } catch (error) {
    console.error("Database Error:", error);
    throw new Error("Gagal ambil data kesehatan.");
  }
}
...
export async function getKesehatanData(query) {
  try {
    ...
    const kesehatan = rows.filter(
      (item) =>
        item.jenis_penyakit.toLowerCase().includes(queryHrfKecil) ||
        item.nama.includes(queryHrfKecil)
    );
    const totalPages = Math.ceil(Number(kesehatan.length) / ITEMS_PER_PAGE);
    // console.log(totalPages);
    return totalPages;
  } catch (error) {
    console.error("Database Error:", error);
    throw new Error("Gagal hitung data dari kesehatan.");
  }
}
...
export async function getKesehatanDataById(idKesehatanUpdate) {
  try {
    ...
    // const dataFromSheet = rows.map((item) => item.tanggal);
    // console.log(dataFromSheet);
    const kesehatanById = rows.filter(
      (item) => item.id_penyakit === idKesehatanUpdate
    );
    return kesehatanById;
  } catch (error) {
    console.error("Database Error:", error);
