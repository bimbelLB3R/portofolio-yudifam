...
export async function getFilteredPerkembanganData(query, currentPage) {
  ...
  try {
      ...
      const filteredRows = rows
        .filter(
          (item) =>
            ...
            item.jenis_perkembangan.toLowerCase().includes(lowerQuery)
        )
        .sort((a, b) => {
          // Pastikan bahwa Anda memiliki properti yang sesuai untuk tanggal di dalam objek 'item'
          const dateA = a.id_perkembangan;
          const dateB = b.id_perkembangan;
          return dateB - dateA;
    ...
    const perkembangans = filteredRows;

    return perkembangans;
  } catch (error) {
    console.error("Database Error:", error);
    throw new Error("Gagal ambil data perkembangan.");
  }
}
...
export async function getPerkembanganData(query) {
  try {
    ...
    const perkembangan = rows.filter(
      (item) =>
        item.jenis_perkembangan.toLowerCase().includes(queryHrfKecil) ||
        item.nama.includes(queryHrfKecil)
    );
    const totalPages = Math.ceil(Number(perkembangan.length) / ITEMS_PER_PAGE);
    // console.log(totalPages);
    return totalPages;
  } catch (error) {
    console.error("Database Error:", error);
    throw new Error("Gagal hitung data dari perkembangan.");
  }
}
...
export async function getPerkembanganDataById(idPerkembanganUpdate) {
  try {
    ...
    // const dataFromSheet = rows.map((item) => item.tanggal);
    // console.log(dataFromSheet);
    const perkembanganById = rows.filter(
      (item) => item.id_perkembangan === idPerkembanganUpdate
    );
    return perkembanganById;
  } catch (error) {
    console.error("Database Error:", error);
    throw new Error("Gagal ambil data perkembangan sesuai id");
  }
}
